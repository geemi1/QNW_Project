...
float sysC;
uint32_t dbElec[8];
uint32_t ececSum[8] = {0,0,0,0,0,0,0,0};
uint8_t timeElec = 0;
...
//				mcuAdc[i] = (uint16_t)((mcuAdc[i]*0.9) + (ptrADC[i]*0.1));//滤波
			...
			for(i = 0;i < 10;i++)	//得出各通道电流实际值
			{
				if(i<4)				//电流 5 6 7 8
					ececSum[4+i] += mcuAdc[i];
				else if(i > 5)		//电流 1 2 3 4 
					ececSum[i-6] += mcuAdc[i];
			}
			timeElec++;
			if(timeElec >= 200)//200ms计算一次
			{
				timeElec = 0;
				for(i = 0;i < 8;i++)
				{
					dbElec[i] = (uint32_t)(ececSum[i]/200);
					ececSum[i] = 0;
				}
			}
			elecIndex ++;
