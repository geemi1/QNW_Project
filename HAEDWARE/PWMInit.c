...
void PWMTask(void *pdata)
{
	...
	while(1)		//监控 PWM 引脚状态、电流状态、步进电机的软启动、软停止、5秒没信息，回复PWM引脚不动
	{
		...
		for(i = 0;i < 8;i++)
		{
			...
			ledPWMFlag = 1;
			pwmOC[i].timeWork ++;
			if(pwmOC[i].timeWork >= 60 && varOperaRun.sysMode != LOAD_TEST_MODE)//3s没数据下发，则关闭此通道
			{
		...
		{
			if(i<6)
				varBXWRun.elecPWM[i]=(uint16_t)(dbElec[i]*25/41);//得到8路PWM电路电流 （mA）
			else
				varBXWRun.elecPWM[i]=(uint16_t)(dbElec[i]*50/41);//得到8路PWM电路电流 （mA）
