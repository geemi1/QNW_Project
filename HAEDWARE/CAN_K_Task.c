...
void CANChannelCheck(u8 ch,u8 res);	//CAN  通道切换、匹配电阻
//诊断功能 CAN K 线数据解析回复
void CANDelay(int z)
{
	int i,j;
...
void FrameDeal(FrameStruct* ptrFrame)
{
	...
	{
		case 0x03://todo: 设置针脚
			CANChannelCheck(0,0);//默认针脚, Debug 时调试程序用
			switch(ptrData[14])
			{
...
//			CANChannelCheck(0,100);
		...
		case 0x14:	//普通 CAN 单帧、多帧
			...
			else				//发送第一包之后的包
			{
				do{
					...
					offset += 8;
					CANDelay(70);//延时是必要的，否则会丢失包，甚至跑飞程序
				}while(offset < datLen);
			}break;
		case 0x15://普通多帧
			break;
		case 0x17://多包，接收下位机发送的30指令
			...
			{
				OSSemPend(canRCV30,20,&err);//等待 ECU 回复0x30包
				if(err != OS_ERR_NONE)
					break;
				do{
					if(canVCIData.canCH == 1||canVCIData.canCH == 3)	 //CAN2 4通道
