...
extern OS_EVENT * BTSet;			//蓝牙配置  消息队列
...
//	u8 wifiFlag = 0;
	...
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)//接收到数据
  	{
		...
		if(varOperaRun.sysMode != LOAD_TEST_MODE)//不是负载测试模式
		{
			if(rx3Data == 0x7E)
			{
				flag7E ++;
				if(flag7E == 2)
				{
					...
					if(frameLen > 1)
					{
						...
						if(pBTRece != NULL)					//内存块申请成功
						{
							...
							varOperaRun.wirelessMode = BT_MODE;
						}
						else 
						{
	...
	if(USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)//接收完一帧数据
	{
		...
		clear3 = USART3->DR;
		clear3 = clear3;
		if(varOperaRun.sysMode == LOAD_TEST_MODE)//负载测试模式
		{
			frameLen = Store_Getlength(receBT_S);	//接收到的总长度
			if(frameLen > 1)
			{
				pBTRece = Mem_malloc(frameLen);
				if(pBTRece != NULL)					//内存块申请成功
				{
					Store_Getdates(pBTRece,receBT_S,frameLen);
					if(OSQPost(BTSet,pBTRece) != OS_ERR_NONE)//向蓝牙设置消息队列推送消息
						Mem_free((void **)(&pBTRece));	//推送不成功需要释放内存块
				}
				Store_Clear(receBT_S);				//舍弃本次接收的数据
			}
			else if(frameLen == 1)
